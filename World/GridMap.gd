...
func make_map_border():
	$Border.resize_border(cell_size.x,width) #assume is a square
...
func pick_building():
	...
	var skyscraper = 16
	var posible_buildings = [17,18]
	var building
	if (randi() % 99) +1 <= chance_of_skyscraper:
		building = skyscraper
	else:
		building = posible_buildings[randi() % posible_buildings.size() - 1 ]
	return building
...
func erase_walls():
	for i in range(width*height*erase_fraction):
		...
		if current_cell & cell_walls[neighbour]:
			...
			var neighbours_walls = get_cell_item(cell.x + neighbour.x,0, cell.z + neighbour.z) - cell_walls[-neighbour]
			neighbours_walls = clamp(neighbours_walls,0,15)
			
			rpc("place_cell",cell.x,cell.z, walls,0)
			rpc("place_cell", (cell + neighbour).x,(cell + neighbour).z, neighbours_walls,0)
...
func record_tile_positions():
	...
	for x in range(0, width):
		for z in range(0, height):
			...
			var current_tile_tyoe = get_cell_item(current_cell.x, 0, current_cell.z)
			if current_tile_tyoe < 15:
